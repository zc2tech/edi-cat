...
export class ParserUtilsBase {
  ...
  static saveParserMap(documentUri?: vscode.Uri, versionKey?: string) {
    let mapParser: [] = ParserUtilsBase.extensionContext.globalState.get(constants.common.MAP_FILE_PARSER);
    if (!mapParser) {
      mapParser = [];
    }
    if (documentUri) {
    ...
    ParserUtilsBase.extensionContext.globalState.update(constants.common.MAP_FILE_PARSER, mapParser);
  }
  ...
  static removeStateVersionKey(from: vscode.Uri): boolean {
    let mapParser: [] = ParserUtilsBase.extensionContext.globalState.get(constants.common.MAP_FILE_PARSER);
    if (!mapParser) {
      return false; // we cannot find the corresponding record
    } else {
      if (!mapParser[from.path]) {
        // still not found
        return false;
      } else {
        mapParser[from.path] = undefined; // removed
        return true;
      }
    }
  }
