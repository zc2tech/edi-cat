 ...
 */
export class Cvt850SalesOrder extends ConverterBase {
    ...
    private _dealGrpN1(grpN1: ASTNode, salesOrderHeader: XMLBuilder) {
        ...
        xmlContact.att('role', MAPS.mapN101[roleCode]);
        xmlContact.ele('Name').txt(this._segVal(N1, 2))
            .att(XML.lang, 'EN');
        let codeQualifier = this._segVal(N1, 3) ?? 'NotExist';
        xmlContact.att('addressIDDomain', MAPS.mapN103[codeQualifier]);
        xmlContact.att('addressId', this._segVal(N1, 4));

        // N2
        let arrN2 = this._dSegs(grpN1, 'N2');
        let postalAddress = xmlContact.ele('PostalAddress');
