...
import { TidyComponentConsumptionDetails, TidyPackaging, TidyShipControl, TidyShipNoticeHeader, TidyShipNoticeItem, TidyShipNoticeItemDetail, TidyShipNoticePortion, TidyTermsOfDelivery } from "../xmlTidy/ShipNotice";
...
export class Cvt_856_ShipNotice extends ConverterBase {
    ...
    public toXML(): string {
        ...
        tShipNoticeHeader.att('fulfillmentType', MAPS.mapBSN640[vBSN07]);
                    ...
                    if (Object.keys(mapHL_T).length > 0) {
                        ...
                        for (let sHL_T in mapHL_T) {  // HL T
                            ...
                            for (let sHL_P in mapHL_P) { // HL P
                                ...
                                this._rpl(this._xmlChild(tTare.ShippingContainerSerialCode, 'ShippingContainerSerialCode')
    ...
    private _dealGrpHL_I(grpHL: ASTNode, tItem: TidyShipNoticeItem) {
        ...
        let sSupplierBatchID: string;
        let sBuyerBatchID: string;
        for (let i = 2; i <= 30; i++) {
            ...
            switch (vQualifier) {
                case 'B8':
                    ...
                    break;
                case 'BP':
                    this._ele2(tItem.ItemID, 'ItemID').ele('BuyerPartID').txt(v);
                    break;
                case 'C3':
                ...
                case 'EN':
                    ...
                    this._ele2(tItem.ItemID, 'ItemID').ele(XML.IdReference).att(XML.domain, 'EAN-13')
                ...
                case 'SN':
                    ...
                    break;
                case 'UP':
                    this._ele2(tItem.ItemID, 'ItemID').ele(XML.IdReference).att(XML.domain, 'UPCConsumerPackageCode')
                        .att(XML.identifier, v);
                    break;
                case 'VP':
                    this._ele2(tItem.ItemID, 'ItemID').ele('SupplierPartID').txt(v);
                    break;
                case 'VS':
                    this._ele2(tItem.ItemID, 'ItemID').ele('SupplierPartAuxiliaryID').txt(v);
                    break;

            } // end  switch(vQualifier)
        } // end for loop i
        let eBatch = tItem.Batch.ele('Batch');
        if (sBuyerBatchID) {
            eBatch.ele('BuyerBatchID').txt(sBuyerBatchID);
        }
        if (sSupplierBatchID) {
            eBatch.ele('SupplierBatchID').txt(sSupplierBatchID);
        }
        ...
        if (SN1) {
            ...
            let eDispatchQuantity = tItem.tPacking.DispatchQuantity.ele('DispatchQuantity');
            tItem.att('quantity', vSN1_2);
            tItem.att('lineNumber', this._segVal(SN1, 1));

            eDispatchQuantity.att(XML.quantity, vSN1_2);
            tItem.UnitOfMeasure.ele(XML.UnitOfMeasure).txt(vSN1_3);
            eDispatchQuantity.ele(XML.UnitOfMeasure).txt(vSN1_3);
        ...
        if (PO4 && (this._vBSN05 == '0004')) {
            ...
            if (vPO4_1 && vPO4_2) {
                eDispatchQuantity.att(XML.quantity,
                    (Number.parseInt(vPO4_1) * Number.parseInt(vPO4_2)).toString());
            }
        ...
        for (let REF of REFs) {
            ...
            switch (vREF1) {
                ...
                case '0L':
                    ...
                    break;
                case 'BT':
                    let sDate = utils.dateStrFromREFDTM(vREF3);
        ...
        for (let DTM of DTMs) {
            ...
            switch (vDTM1) {
                ...
                case '511':
                    // BestBeforeDate
                    arrRetailChild[0] = utils.dateStrFromCCYYMMDD(vDTM2);
                    break;
            }
        }
    ...
    private _dealGrpHL_T(grpHL: ASTNode, tTare: TidyPackaging) {
        ...
        tTare.Description.ele(XML.Description).att('type', 'Package').att(XML.lang, 'en');
    ...
    private _dealGrpHL_S(grpHL: ASTNode, tShipHeader: TidyShipNoticeHeader, tControl: TidyShipControl) {
        ...
        let ePackaging = tShipHeader.Packaging.ele('Packaging');
        // MEA
        let MEAs = this._dSegs(grpHL, 'MEA');
        for (let MEA of MEAs) {
            let vMEA1 = this._segVal(MEA, 1);
            ...
            if (vMEA1 == 'PD' && vMEA2 != 'G' && vMEA2 != 'VWT' && vMEA2 != 'WT') {
                let eDim = ePackaging.ele('Dimension');
        ...
        for (let TD1 of TD1s) {
            ...
            if (vTD1_1) {
                ...
                switch (vTD1_6) {
                    case 'G':
                        ePackaging.ele('Dimension').att('quantity', vTD1_7).att('type', 'grossWeight')
                            .ele(XML.UnitOfMeasure)
                            .txt(MAPS.mapUOM355[vTD1_8]);
                        break;
                    case 'N':
                        ePackaging.ele('Dimension').att('quantity', vTD1_7).att('type', 'weight')
                            .ele(XML.UnitOfMeasure)
                            .txt(MAPS.mapUOM355[vTD1_8]);
                        break;
                    case '':
                        ePackaging.ele('Dimension').att('quantity', vTD1_9).att('type', 'grossVolume')
                            .ele(XML.UnitOfMeasure)
        ...
        for (let TD5 of TD5s) {
            ...
            tShipHeader.ServiceLevel.ele('ServiceLevel').att('xml:lang', 'en')
                .txt(MAPS.mapTD512[vTD5_12]);
            tShipHeader.ServiceLevel.ele('ServiceLevel').att('xml:lang', 'en')
                .txt(MAPS.mapTD512[vTD5_13]);
            tShipHeader.ServiceLevel.ele('ServiceLevel').att('xml:lang', 'en')
                .txt(MAPS.mapTD512[vTD5_14]);
        } // end loop TD5s
                    ...
                    case 'FOB05':
                        ...
                        if (FOB) {
                            ...
                            if (!MAPS.mapFOB05[vFOB05]) {
                                tTermOfD.TransportTerms.ele('TransportTerms').att('value', vREF3);
                            }
                        }
                        break;
            ...
            switch (vDTM1) {
                ...
                case '118':
                    tShipHeader.Extrinsic.ele(XML.Extrinsic).att('name', 'PickUpDate')
                        .txt(utils.dateStrFromDTM(vDTM2, vDTM3, vDTM4));
                    break;
            }
        }
        ...
        if (FOB) {
            ...
            if (vFOB1 == 'DF') {
                if (arrPaymentInclude.includes(vFOB7)) {
    ...
    protected _fill_HL_S_N1GrpDA(grpN1: ASTNode, tShipHeader: TidyShipNoticeHeader, tTermOfD: TidyTermsOfDelivery) {
        ...
        eAddr.ele('Name').att(XML.lang, 'en').txt(this._segVal(N1, 2));
        eAddr.att('addressID', this._segVal(N1, 4));
        eAddr.att('addressIDDomain', MAPS.mapN103[this._segVal(N1, 3)]);
        ...
        for (let N2 of N2s) {
            if (!ePostalAddress) {
                ePostalAddress = eAddr.ele('PostalAddress');
            }
            ...
            if (vPER1 == 'CN') {
                for (let i = 3; i <= 7; i = i + 2) {
                    ...
                    switch (this._segVal(PER, i)) {
                        case 'TE':
                            eAddr.ele('Phone').att('name', vPER2).ele('TelephoneNumber')
                            ...
                            break;
                        case 'FX':
                            eAddr.ele('Fax').att('name', vPER2).ele('TelephoneNumber')
                                .ele('CountryCode').att('isoCountryCode', '').txt(this._arrVal(arrV, 0))
                                .up().ele('AreaOrCityCode').txt(this._arrVal(arrV, 1))
                                .up().ele('Number').txt(this._arrVal(arrV, 2))
                            break;
                        case 'EM':
                            eAddr.ele('Email').att('name', vPER2)
                                .txt(v);
                            break;
                        case 'UR':
                            eAddr.ele('URL').att('name', vPER2)
                                .txt(v);
                            break;
                    }
                }
    ...
    protected _fill_HL_S_N1Grp1(grpN1: ASTNode, tShipHeader: TidyShipNoticeHeader) {
        ...
        eContact.att('role', MAPS.mapN101[x12Role]);
        eContact.ele('Name').att(XML.lang, 'en').txt(this._segVal(N1, 2));
        let codeQualifier = MAPS.mapN103[this._segVal(N1, 3)];
        if (codeQualifier) {
            eContact.att('addressID', this._segVal(N1, 4))
            eContact.att('addressIDDomain', codeQualifier);
        ...
        if (arrN2 && arrN2.length > 0) {
            for (let segN2 of arrN2) {
                let ePostalAddress = this._ele2(eContact, 'PostalAddress')
        ...
        if (arrN3 && arrN3.length > 0) {
            for (let segN3 of arrN3) {
                ePostalAddress = this._ele2(eContact, 'PostalAddress')
        ...
        this._X12N4(grpN1, eContact);

        // REF
        let REFs = this._dSegs(grpN1, 'REF');
        REFs = REFs ?? [];
        let fIdRefs = fragment();
        for (let REF of REFs) {
            let vREF1 = this._segVal(REF, 1);
            let vREF2 = this._segVal(REF, 2);
            let vREF3 = this._segVal(REF, 3);
        ...
        if (PER) {
            ...
            if (vPER1 == 'CN') {
                for (let i = 3; i <= 7; i = i + 2) {
                    ...
                    switch (this._segVal(PER, i)) {
                        case 'TE':
                            eContact.ele('Phone').att('name', vPER2).ele('TelephoneNumber')
                                .ele('CountryCode').att('isoCountryCode', this._arrVal(arrV, 0)).txt(MAPStoXML.mapCountry[this._arrVal(arrV, 0)])
                                .up().ele('AreaOrCityCode').txt(this._arrVal(arrV, 1))
                                .up().ele('Number').txt(this._arrVal(arrV, 2))
                            break;
                        case 'FX':
                            eContact.ele('Fax').att('name', vPER2).ele('TelephoneNumber')
                                .ele('CountryCode').att('isoCountryCode', '').txt(this._arrVal(arrV, 0))
                                .up().ele('AreaOrCityCode').txt(this._arrVal(arrV, 1))
                                .up().ele('Number').txt(this._arrVal(arrV, 2))
                            break;
                        case 'EM':
                            eContact.ele('Email').att('name', vPER2)
                                .txt(v);
                            break;
                        case 'UR':
                            eContact.ele('URL').att('name', vPER2)
                                .txt(v);
                            break;
                    }
                }
        ...
        eContact.import(fIdRefs); // IDRef should be at the end of Contact Element
